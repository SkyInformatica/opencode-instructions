...
> **Status OpenCode V2:** o installer upstream (`bin/install.js --only opencode`) ainda escreve plugin na API V1 (hooks `session.created` + `tui.prompt.append`) e **o plugin não roda no V2**. Porém o Caveman é resiliente: a regra always-on vem do bloco em `AGENTS.md` (escrito pelo mesmo installer), então no V2 o modo caveman continua valendo; só os comandos de troca de intensidade dependem do plugin. Ao instalar em máquina V2: instale skill + bloco AGENTS.md normalmente e avise o usuário que o plugin/ comandos `/caveman <modo>` só funcionarão quando o upstream portar para a API V2. O installer pode tentar patch no array `plugin` do opencode.json — se ele criar/alterar, prefira confirmar o array `plugins` (V2) também, sem duplicar entradas.
...
4. Verifique a instalação:
   ...
   - Se a máquina roda V2: confirme o aviso ao usuário de que o plugin do Caveman ainda é V1 (não ativo no V2) até o upstream portar; na prática a regra do AGENTS.md mantém o modo ativo.
