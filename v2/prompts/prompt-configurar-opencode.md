...
3b. **Se o escopo incluir agents**, baixe os agents selecionados da pasta agents/ do repositório para a pasta global de agents:
   https://github.com/SkyInformatica/opencode-instructions/tree/main/v2/agents

   Liste o conteúdo da pasta agents via GitHub API:
   https://api.github.com/repos/SkyInformatica/opencode-instructions/contents/v2/agents
...
4. **Se o escopo incluir regras**, configure o opencode.json global:
   - Se %USERPROFILE%\.config\opencode\opencode.json já existir, leia o conteúdo atual.
     - Garanta que o array "instructions" contenha o padrão global (apenas uma vez, sem duplicar):
      ...
      - Se "model" não estiver definido, defina como `opencode/deepseek-v4-flash`. No formato nativo V2, o antigo `small_model` virou `agents.title.model` (modelo do agente de título); defina também se quiser o mesmo modelo para o title: `"agents": { "title": { "model": "opencode/deepseek-v4-flash" } }`. Se o arquivo ainda usar `small_model` (sintaxe V1), ele continua aceito pelo V2, mas prefira `agents.title.model`.
   ...
   - Se não existir, crie usando como modelo:
     https://github.com/SkyInformatica/opencode-instructions/blob/main/v2/global/opencode.json
      Adapte model (`opencode/deepseek-v4-flash`), agents.title.model (antigo small_model, `opencode/deepseek-v4-flash`), instructions e `"shell": "bash"` conforme necessário.
