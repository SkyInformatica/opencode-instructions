...
> **Status OpenCode V2:** o upstream está portando o plugin para V2 (`hooks/opencode/rtk-v2.ts` + flag `--opencode-v2`, PR em aberto no repo rtk-ai/rtk). A versão atual usa a API V1 (`tool.execute.before`) e **não roda no V2**. Antes de instalar, verifique se a versão do rtk instalada já suporta V2: `rtk init --help` deve listar `--opencode-v2`. Preferência por versão da máquina: se V1 → `rtk init -g --opencode`; se V2 com suporte → `rtk init -g --opencode-v2`; se V2 sem suporte → avise o usuário em vez de instalar algo que não carregará.
...
2. Verifique se o RTK já está instalado para o OpenCode antes de qualquer alteração:
   ...
   - O OpenCode já tem um plugin RTK configurado? Verifique se o `%USERPROFILE%\.config\opencode\opencode.json` já contém referência a RTK no array de plugins (`plugin` no V1 ou `plugins` no V2).

   Se tudo já existir, pule para o passo 4 (verificação). Não reinstale nem duplique nada.

3. Determine a flag conforme a versão do OpenCode e do rtk:
   - `rtk init --help` — verifique se a versão instalada do rtk lista `--opencode-v2`.
   - Máquina com OpenCode V2 **e** rtk com `--opencode-v2`: use
     rtk init -g --opencode-v2
   - Caso contrário (V1 ou rtk sem suporte V2): use
     rtk init -g --opencode

   Observações:
   - O flag `-g` instala globalmente (não apenas no projeto atual).
   - O flag `--opencode` configura o plugin TS para OpenCode V1 (`tool.execute.before`); `--opencode-v2` configura o plugin nativo V2 (`hooks/opencode/rtk-v2.ts`, API `Plugin.define`).
