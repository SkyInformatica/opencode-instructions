...
Nomes de funções, métodos, classes, arquivos e variáveis devem expressar a intenção de domínio do que fazem, e não detalhes internos de implementação, transporte, infraestrutura, tratamento de exceções, status HTTP, mecanismo de persistência, estratégia de retry ou forma de consulta.
...
- Quando a falha faz parte do comportamento esperado da camada, isso continua implícito e não entra no nome. Exceções, status HTTP, logs, `commit`, `flush`, cache, fallback, retry e outros detalhes operacionais devem aparecer no corpo da implementação, não na assinatura nominal.
...
- Tratar erros com mensagens objetivas e status HTTP coerente; manter logs neutros para serviços externos.
...
- Ao corrigir bug ou alterar regra de negócio, contrato, mapeamento ou integração, atualizar/adicionar testes focados quando viável.
- Nomear testes pelo comportamento esperado e cobrir bordas/cenários de erro relevantes sem criar testes de baixo valor.
- Antes de concluir mudança relevante, executar validações compatíveis com o contexto (diagnósticos, lint e/ou testes) sem interromper fluxos ativos sem necessidade.
- Para correção de bug ou mudança de regra, escrever primeiro o teste que reproduz/exige o comportamento e então fazê-lo passar; iterar até o critério de sucesso ser verificável e verificado, em vez de validar só ao final.
